...
def main():
    ...
    parser.add_argument('-j', '--jobtype', type=str, choices=['bilby', 'pink'], default='bilby')
    ...
    filetag='_pink' if args.jobtype=='pink' else ''
    script_dir = Path(__file__).resolve().parent.parent
    bash_dir = script_dir / 'bash_scripts'

    for i in range(0, 10000, 100):
        start, end = f'{i:05d}', f'{i+99:05d}'
        filename = bash_dir / f'{datatag}_run_bilby{filetag}_id{start}-{end}.sh'
        script_content = f'''#!/bin/bash
source /n/home04/kyoon/miniforge3/etc/profile.d/conda.sh
conda activate ssm
BASE_DIR="/n/holystore01/LABS/iaifi_lab/Lab/kyoon"
python ${{BASE_DIR}}/ssm_regression/toy/call_fitting.py -t {args.datatype} -j {args.jobtype} -b {start} {end} \\
